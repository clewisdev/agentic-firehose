...
Brussee also publishes a standalone `caveman` skill (distinct from caveman-code) which is purely an output style instruction: "talk like caveman." This is the version that appears in an internal enterprise team's shared skill library and Matt Pocock's skills collection. The skill reduces output tokens by 65% through compressed prose, with guardrails preserved for security warnings, irreversible operations, and confusing multi-step sequences.
